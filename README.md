...
5. **服务端并发限制的处理**：Cloud Control API 对 `CloudIdentity/Organization` 类资源有服务端并发锁。工程采用 Terraform 内置的 module 层 `depends_on` 显式串联**层与层**（`organization_units` → `organization_accounts`、`cloud_identity_users` → `cloud_identity_groups`、`permission_sets` + 所有上游 → `permission_set_assignments`），确保上层完全就绪后下层才开始，避免跨层并发冲突。
   
   同一层内部（`for_each` 展开的多个实例）Terraform 仍按 `parallelism` 并行创建。若首次 apply 因偶发 `ConcurrentException` 部分资源失败，**直接重跑 apply 即可幂等收敛**（Terraform 会跳过已建、重试未建）。若资源实例数较多（10+），建议依赖运行环境降低 `-parallelism` 或分批 apply。
...
| `ConcurrentException: Concurrent request exception` | Cloud Control API 服务端并发限制（同层多实例并行触发） | 直接重跑 `terraform apply` 幂等收敛；如资源多请降低 `-parallelism` |
| `AlreadyExists: ... name already exists` | 上一次 apply 因并发失败但服务端实际已创建，需 import 拉入 state | `terraform import '<address>' <id>` 拉入 state 后重跑 apply |
